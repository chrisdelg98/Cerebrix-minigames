...
import { type LightsOutConfig, type LightsOutMove, type LightsOutState } from './types';
 ...
 * de llegar a cinco.
 */
const CONFIGS: Record<Difficulty, LightsOutConfig> = {
  1: { size: 3, clicks: 3 },
  2: { size: 4, clicks: 5 },
  3: { size: 5, clicks: 8 },
  4: { size: 5, clicks: 14 },
...
export const lightsOutEngine: GameEngine<LightsOutState, LightsOutMove, LightsOutConfig> = {
  ...
  deserialize(raw) {
    return JSON.parse(raw) as LightsOutState;
  },
};
